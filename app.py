...
if page == "實驗數據管理":
    ...
    *   **管理步驟：** 在清單中展開您的計畫，並可逐條增加測試步驟，系統將為您妥善保管每一步驟序列。
    ...
    ### 4. 呼叫 AI 助理
    在「AI 科研助理」頁中，系統已經升級串接高階大型語言模型！您可以直接用平常講話的自然語言詢問任何科研問題，或者是使用以下幾種標準格式調用系統內建模組：
    *   **查詢專屬儀器：** 輸入 `儀器使用 [儀器名稱]` (例如: `儀器使用 顯微鏡`)。
    *   **規劃實驗：** 輸入 `設計實驗流程 [研究目標]`。
    *   **拆解問題：** 輸入 `拆分問題 [龐大研究命題]`。
    *   **生成規範：** 輸入 `標準流程 [實驗細項]` 生成安全與操作守則 (SOP)。
    *   **自由問答：** 輸入任何其他文字，例如 `請跟我解釋什麼是聚合酶連鎖反應 (PCR)？`，AI 會根據其豐富的知識庫為您解答。
    """)
